...
// load data
d3.json('./data/scores.json', function (err, data) {
  var records = data.records;

  // xScale
  var xScale = d3.scaleBand().domain(records.map(function (d) {
    return d.name;
  ...
  var render = function render() {
    var subject = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'math';

    var t = d3.transition().duration(1000);

    var update = svg.selectAll('rect').data(records.filter(function (d) {
      return d[subject];
    }), function (d) {
      return d.name;
    });

    update.exit().transition(t).attr('y', height).attr('height', 0).remove(); // get rid of elements we no longer need

    yScale.domain([0, d3.max(records, function (d) {
      return d[subject];
    })]);
    yAxis.transition(t).delay(250).call(d3.axisLeft(yScale));

    update.transition(t).delay(250).attr('y', function (d) {
      return yScale(d[subject]);
    }).attr('height', function (d) {
      return height - yScale(d[subject]);
    });

    update.enter().append('rect').attr('y', height).attr('height', 0).attr('x', function (d) {
      return xScale(d.name);
    }).attr('width', function (d) {
      return xScale.bandwidth();
    }).transition(t).delay(update.exit().size() ? 1000 : 0).attr('y', function (d) {
      return yScale(d[subject]);
    }).attr('height', function (d) {
      return height - yScale(d[subject]);
    });
